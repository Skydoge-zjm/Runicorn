...
│  │ - metrics 表       │ - archive/ (已保存资产)         │   │
│  │ - assets 索引      │ - runs/ (元数据、日志、资产)    │   │
│  └────────────────────┴─────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────┘
```
...
- SQLite 支撑的 run 查询：在 10,000+ 实验规模下可快 100 倍
- 连接池和缓存
- 高效索引
...
**关键类 / 辅助函数**:
- `Run`: 实验上下文和生命周期
- `snapshot_workspace()`: 创建工作区快照归档
- Run 辅助函数: `log_config()`、`log_dataset()`、`log_pretrained()`
...
**组件**:
- **路由模块**: 按功能组织（runs、metrics、assets、remote、import/export）
- **服务层**: 业务逻辑抽象
- **中间件**: CORS，速率限制，日志
...
**职责**: 记录可复用的运行上下文和归档文件，而不是提供独立的公开 Artifact 注册表

**组件**:
- **运行清单** (`assets.json`): 每个运行自己的嵌套资产元数据
- **归档存储** (`archive/`): 已保存文件、manifest 与 blob 存储
- **SQLite 资产索引** (`assets`、`run_assets`): 跨运行查询与引用计数
- **清理辅助工具** (`cleanup.py`): 孤立资产检测与永久删除支持

**设计模式**: 每运行清单 + 共享归档/索引

---

### 6. Assets 系统（引入于 v0.6.0）
...
| 指标 | 已测试 | 实际限制 | 注释 |
|------|--------|----------|------|
| 实验数 | 100,000 | ~500,000 | 需要 SQLite 支撑的列表路径 |
| 已归档资产 | 10,000 | ~50,000 | 归档索引 / 去重池大小 |
| 指标点 | 1000万 | ~1亿 | 每个实验 |
...
- **[SSH_BACKEND_ARCHITECTURE.md](SSH_BACKEND_ARCHITECTURE.md)** - SSH 后端设计
