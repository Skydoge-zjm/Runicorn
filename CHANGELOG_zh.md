...
- **修复**: 调整 metrics identity 处理，避免旧 schema 路径下同一指标快速连续写入时覆盖更早数据点
- **改进**: 清理 Remote API 与旧路径兼容面，使当前统一 Remote Viewer 接口与实现边界更一致
- **改进**: 文档重新对齐当前远程 session 模型、API 路由与桌面构建流程

### 安全与兼容性

- **改进**: 收紧历史 XOR 凭据迁移边界，在保留兼容读取能力的同时减少意外 fallback 风险
- **改进**: 已保存凭据与远程连接相关行为按当前安全预期做了进一步加固和澄清

### 测试、CI 与构建加固

- **改进**: 扩充前端测试脚手架与 smoke 覆盖，补齐当前 loading、baseline 与 remote 工作流验证
- **改进**: 扩展 CI 表面，引入分层 desktop build 配置并加强 sidecar 打包链路校验
- **改进**: 桌面打包流程更可复现，构建控制更清晰，并补齐中英文构建说明

## [0.7.0] - 2026-03-15

### Remote Viewer 与 SSH

- **新增**: 分步骤的 Remote Viewer 配置流程，支持保存服务器、保存连接配置与更清晰的进度反馈
- **新增**: 在 UI 中确认主机密钥，并管理 known-hosts
- **新增**: 健康监控、`reconnecting` / `degraded` 会话状态、空闲自动退出，以及 SSH 连接自动清理
- **新增**: OpenSSH 优先行为与密码认证支持，后端选择逻辑更合理
- **改进**: 通过缓存与批量检测加速远程环境探测
- **改进**: 远程会话围绕单一 Stop 操作进行管理，交互更简洁

### Web UI 与实验工作流

- **改进**: 更专业的默认视觉风格，去除过度装饰性的界面效果
- **新增**: 更完善的对比流程，支持 URL 持久化 compare 状态、图表联动高亮，以及在对比面板中显示 tags
- **新增**: ZIP 导出，以及带冲突预览的导入流程
- **新增**: 统一的回收站流程，覆盖运行和文件夹级删除
- **改进**: 路径树工作流与文件夹操作，视觉表现也更完善
- **改进**: 设置抽屉重组、主题感知弹窗，以及更一致的暗色模式体验
- **新增**: 资产页、远程页、性能页、实验详情页等 UI 文档补充完成

### 日志、性能与监控

- **新增**: 面向大日志场景的虚拟滚动
- **新增**: 后端采集的 GPU 遥测历史，以及相关采集频率和保留设置
- **改进**: 长日志、ANSI 输出与实时日志体验
- **改进**: 图表、表格与主题相关交互的整体细节

### SDK、兼容层与存储

- **新增**: 对 ImageNet 风格 meters、TensorBoard `SummaryWriter` 与 `tensorboardX` 的兼容支持
- **改进**: SDK 文档和示例更新到当前的路径组织与本地优先工作流
- **改进**: 统一的 SQLite 存储 / 索引路径，Viewer 优先读取 SQLite，同时保留文件系统回退
- **改进**: config、storage 与 client 相关代码结构进一步统一和清理

### 稳定性与 Bug 修复

- **修复**: 项目级 bug audit 暴露出的多批后端 / 前端问题（涉及 run 生命周期、资产、远程会话和 UI 行为）
- **修复**: Viewer 同步线程与 SQLite 访问相关的数据库锁和关闭竞态问题
- **修复**: 旧 schema / 配置迁移问题，以及多个 Remote 会话边界情况
- **改进**: 单元测试、集成测试、E2E 与前端测试覆盖显著增强

### 桌面端

- **新增**: 桌面版 Remote Viewer 会话可在原生窗口中打开
- **改进**: 使用系统浏览器打开外部链接的桌面流程
